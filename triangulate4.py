...
#            eg: python triangulate4.py data/extri_data/images/2026-08-21_10/ data/intri_data/params/2026-08-21_10/ (outdir auto created)
#            or python triangulate4.py data/extri_data/images/2026-08-21_10/ data/intri_data/params/2026-08-21_10/ --out_dir data/extri_data/params

# OUTPUTS: multi_camera_calib.npz, 5 data plots
#         if path not specified, these files are save in extri_data/params/{date and time of extri_data images}
...
# import csv
# from aniposelib.boards import CharucoBoard as AniCharucoBoard, merge_rows, extract_points
# from aniposelib.cameras import Camera, CameraGroup
# from mpl_toolkits.mplot3d import Axes3D 

# def _compat_estimatePoseCharucoBoard(charucoCorners, charucoIds, board, cameraMatrix, distCoeffs,
#                                       rvec=None, tvec=None, useExtrinsicGuess=False,
#                                       max_reproj_px=2.0, ambiguity_ratio=3.0):
#     """Drop-in replacement for the removed cv2.aruco.estimatePoseCharucoBoard,
#     using solvePnPGeneric + IPPE with the same ambiguity gating as pose_from_corners,
#     so aniposelib's initial extrinsics guess isn't seeded with flipped/bad poses."""
#     ids_flat = np.array(charucoIds).flatten().astype(int)
#     if len(ids_flat) < 4:
#         return False, None, None

#     obj_points_all = board.getChessboardCorners()
#     obj_pts = obj_points_all[ids_flat]
#     img_pts = np.array(charucoCorners).reshape(-1, 2)

#     try:
#         n_sol, rvecs, tvecs, _ = cv2.solvePnPGeneric(
#             obj_pts, img_pts, cameraMatrix, distCoeffs, flags=cv2.SOLVEPNP_IPPE)
#     except cv2.error:
#         return False, None, None
#     if n_sol < 1:
#         return False, None, None
...
#         proj, _ = cv2.projectPoints(obj_pts, rv, tv, cameraMatrix, distCoeffs)
...
# cv2.aruco.estimatePoseCharucoBoard = _compat_estimatePoseCharucoBoard
...
# def build_ani_row(framenum, fname, corner_dict, n_board_points):
#     """Build a row in aniposelib's expected format, using YOUR OWN working detector
#     (cv2.aruco.CharucoDetector) instead of aniposelib's version-broken internal one."""
#     ids = sorted(corner_dict.keys())

#     if ids:
#         corners = np.array([corner_dict[i] for i in ids], dtype=np.float64).reshape(-1, 1, 2)
#         ids_arr = np.array(ids, dtype=np.int32).reshape(-1, 1)
#     else:
#         corners = np.zeros((0, 1, 2), dtype=np.float64)
#         ids_arr = np.zeros((0, 1), dtype=np.int32)

#     filled = np.full((n_board_points, 1, 2), np.nan, dtype=np.float64)
#     for cid in ids:
#         filled[cid, 0, :] = corner_dict[cid]

#     return {'framenum': framenum, 'corners': corners, 'ids': ids_arr,
#             'fname': fname, 'filled': filled}
...
#         path = args.images_dir.split('/')
#         assert len(path) == 4, (f"Wrong path input for images you might have an extra / at the end")
#         out_dir = f'data/extri_data/params/{path[-1]}'
...
#         # ---- aniposelib board (separate object, same physical board as your cv2 one) ----
#     # NOTE: aniposelib wants square_length/marker_length in real units (mm here),
#     # not meters like your cv2 board — convert.
#     ani_board = AniCharucoBoard(
#         squaresX=SQUARES_X, squaresY=SQUARES_Y,
#         square_length=CHECKER_SIZE * 1000,
#         marker_length=MARKER_SIZE * 1000,
#         marker_bits=4, dict_size=50,
#     )

#     # ---- Seed aniposelib Camera objects with YOUR already-computed intrinsics ----
#     ani_cameras = []
#     for c in range(n_cams):
#         cam = Camera(matrix=Ks[c], dist=Ds[c], size=image_size, name=f"cam{c+1:02d}")
#         ani_cameras.append(cam)
#     cgroup = CameraGroup(ani_cameras)

#     # ---- Detect board in each camera's image folder using aniposelib's own detector ----
#     # print("\nDetecting board across all cameras (aniposelib)...")
#     n_board_points = GRID_W * GRID_H
#     print("\nDetecting board across all cameras (non anipose detector)...")
#     all_rows = []
#     for c in range(n_cams):
#         img_paths = list_images(image_dirs[c])
#         rows = []
#         for framenum, path in enumerate(img_paths):
#             im = cv2.imread(path)
#             if im is None:
#                 continue
#             det = detect_corners(im, detector)   # your existing, proven detector
#             if len(det) >= MIN_CORNERS:
#                 rows.append(build_ani_row(framenum, path, det, n_board_points))
#         print(f"  cam{c}: {len(rows)}/{len(img_paths)} frames with detections")
#         all_rows.append(rows)
    
#     # print(all_rows)

#     # ---- Joint bundle adjustment across ALL cameras simultaneously ----
#     # init_intrinsics=False: keep YOUR already-calibrated K/dist, don't refit them here.
#     # init_extrinsics=True: estimate a starting R/T per camera from shared detections,
#     # then bundle_adjust_iter jointly refines every camera's extrinsics using every
#     # 2D observation at once — not chained pairwise like the BFS approach.
#     error = cgroup.calibrate_rows(
#         all_rows, ani_board,
#         init_intrinsics=False,
#         init_extrinsics=True,
#         verbose=True,
#     )
#     print(f"\nBundle-adjusted mean reprojection error: {error:.4f} px")

#     # ---- Pull R/T back out in the format the rest of your script expects ----
#     world_R, world_T = {}, {}
#     for c, cam in enumerate(cgroup.cameras):
#         R, _ = cv2.Rodrigues(cam.get_rotation())
#         T = cam.get_translation().reshape(3, 1)/1000.0
#         world_R[c] = R
#         world_T[c] = T

#     # aniposelib doesn't anchor cam0 at the identity by default — re-anchor everything
#     # to cam0 so downstream code (which assumes cam0 = world origin) still works.
#     # R0_inv = world_R[0].T
#     # T0 = world_T[0].copy()
#     # for c in range(n_cams):
#     #     world_R[c] = R0_inv @ world_R[c]
#     #     world_T[c] = R0_inv @ (world_T[c] - T0)
...
#     print(f"\nPose chain (world = cam0), after bundle adjustment:")
#     for c in sorted(world_R):
#         pos = (-world_R[c].T @ world_T[c]).flatten() * 1000
#         print(f"  cam{c}: position [{pos[0]:8.1f}, {pos[1]:8.1f}, {pos[2]:8.1f}] mm")

#     for cam in cgroup.cameras:
#         cam.set_translation(cam.get_translation() / 1000.0)
    
#     # Save aniposelib's native format too, useful for anipose directly
#     cgroup.dump(os.path.join(out_dir, "calibration.toml"))
#     print(f"Saved aniposelib calibration.toml to {out_dir}")

#     # ---- Still need frame_corners for your Pass 2 validation below ----
#     # Rebuild it from your own cv2-based detector so Pass 2 stays unchanged
#     # and gives a fair before/after comparison against your BFS run.
#     frame_corners = {}
#     for i in range(n_frames):
#         corners = {}
#         for c in range(n_cams):
...
#     npz_path = os.path.join(out_dir, "multi_camera_calib.npz")
#     payload = {}
#     for c in range(n_cams):
#         payload[f"K{c}"] = Ks[c]
#         payload[f"D{c}"] = Ds[c]
...
#     # ---- CSV ----
#     with open(os.path.join(out_dir, "per_frame_results.csv"), "w", newline="") as f:
#         w = csv.writer(f)
...
#     # fig.tight_layout()
#     # fig.savefig(os.path.join(out_dir, "edge_baselines.png"), dpi=150)
...
#     ax.grid(alpha=0.3)
#     fig.tight_layout()
#     fig.savefig(os.path.join(out_dir, "spacing_error_per_frame.png"), dpi=150)
...
#     ax.legend()
#     fig.tight_layout()
#     fig.savefig(os.path.join(out_dir, "spacing_histogram.png"), dpi=150)
...
#         cam_pts.append(pos)
#         ax.scatter(*pos, s=90, marker="^", label=f"cam0{c}")
#         draw_axes(ax, world_R[c].T, pos, length=0.08, label=f"cam0{c}")
...
#     fig.tight_layout()
#     fig.savefig(os.path.join(out_dir, "rig_3d.png"), dpi=150)
